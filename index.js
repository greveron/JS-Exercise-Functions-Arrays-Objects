...
function getName(newPerson) {
  ...
  return `hello, my name is ${newPerson.name}`/* code here */
}
...
*/
function makeSmartPerson(Person     /* code here */) {
  const test = {
    name: Person,
   sum (num1, num2){
return num1+num2  
...
function getCarInfoByIndex(inventory, index) {
  ...
  return `This is a ${key.car_make} ${key.car_model}`
}
...
function getCarInfoById(inventory,id) {
 let Car1 = inventory[id-1];
 return `This is a inventory ${Car1.car_make} ${Car1.car_model}`;
...
let inveyear = []  
 for (let i=0; i<inventory.length; i++ ){
   inveyear[i] = inventory[i].car_year;
}
  return inveyear;
}
